...
window.addEventListener('load', () => {
  ...
  // --- Safari Hero Video Autoplay Fix ---
  const heroVideo = document.getElementById('heroVideo');
  if (heroVideo) {
    // Force play — required for Safari which may ignore the autoplay attribute
    heroVideo.play().catch(() => {
      // Autoplay blocked (e.g. Safari low-power mode).
      // Retry on first user interaction so the video starts as soon as possible.
      const tryPlay = () => {
        heroVideo.play().catch(() => { });
    ...
    heroVideo.addEventListener('ended', () => {
      // Only auto-scroll if the user hasn't already scrolled past the hero
      const heroHeight = heroVideo.closest('section').offsetHeight;
...
if (gbForm) {
  gbForm.addEventListener('submit', function (e) {
    ...
    const nameField = this.querySelector('[name="from_name"]');
    ...
    emailjs.send('service_7b4vsue', 'template_1o2j4ws', templateParams)
      .then(() => {
      ...
      .catch((err) => {
        ...
        const text = err && err.text ? err.text : JSON.stringify(err);
        btn.innerHTML = `Error ${status}: ${text}`;
        setTimeout(() => {
          btn.innerHTML = originalText;
          btn.style.pointerEvents = 'auto';
        }, 5000);
